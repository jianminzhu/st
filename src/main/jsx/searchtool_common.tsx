        ...
        try {
            ...
            if (chuizhi != defaultChuizhi) {
                if ($.trim(tplStr) == "") {
                    ...
                    if (chuizhi == "zhihu" || chuizhi == "weixin") {//由于知乎和微信则使用pc的接口，所以将接口全部变更为pc的
                        p._pc_wap = "pc";
                    }
...
class SearchToolErrorUtil {
    ...
    constructor(timeout) {
        ...
        const jerror = jbody.find("#___error__");

        if (jerror.size() == 0) {
            jerror = $(`<div class="error-tip" style="display: none" id="___error__"></div>`);
            jbody.append(jerror);
        }
        const jinfo = jbody.find("#___info__");
        if (jinfo.size() == 0) {
...
function searchtool_ReactSelectRender(eid, items, qcb, width) {
    ...
    ReactDOM.render(<RSelect {...prop}/>, document.getElementById(eid))
    return prop;
}
...
var iframeDialog = function (dialogId: string, url: string) {
...
(function () {
    $(function () {
        if (SEARCHTOOLS_ISCHUIZHI()) {//如果是垂直则页面类型参数
            $('#d_page_type').show();
            if (SEARCHTOOLS_ISCHUIZHI()) {
                ...
                $.ajax({
                    ...
                    if (res) {
                        ...
                        if (navIdStr.length <= 6 && navIdStr.endsWith("0301")) {//如果是菜单【SA】的【query点击分析】则不加全部
        ...
        if (jbatchExport.size() > 0) {
            let dialogId = "a_export_batch_togger";
